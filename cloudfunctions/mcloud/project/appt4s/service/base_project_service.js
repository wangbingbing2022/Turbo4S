...
class BaseProjectService extends BaseService {
	...
	async initSetup() {
		...
		let F = (c) => 'bx_' + c;
		const INSTALL_CL = 'setup_appt4s';
		const COLLECTIONS = ['setup', 'admin', 'log', 'activity_join', 'activity', 'comment', 'day', 'fav', 'join', 'meet', 'news', 'temp', 'user'];
		const CONST_PIC = '/images/cover.gif';

		const NEWS_CATE = '1=公告通知,2=养车知识';
		const MEET_TYPE = '1=保养预约,2=维修预约,3=试驾预约';
